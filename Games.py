...
def GetGames(limit_games = 20): # Tạo biến limit_news để lấy số lượng tin mà mình cần thôi
    ...
    article = soup.select("article.item-news", limit=limit_games) # Tách dữ liệu phần thẻ article ra

    listArticle = []
    for element in article:
        title = element.select("h3.title-games > a") # Lấy phần thẻ chứa title
